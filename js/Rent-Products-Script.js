...
auth.onAuthStateChanged(async (user) => {
  if (user) {
    ...
    async function HandleRenting(productType, productID) {
      ...
      const acknowledgementCheck = document.getElementById(
        "acknowledgement-check"
      );

      if (!acknowledgementCheck.checked) {
        alert(
          "Please read and agree to acknowledgement of the terms of rental"
        );
        return;
      }
      ...
      if (totalAmount) {
        ...
        if (productType === "machinery") {
          ...
          infoOwner.textContent = newData.ownerName;
      ...
      get(productRef).then((snapshot) => {
        if (snapshot.exists()) {
          ...
          if (productType === "machinery") {
            ...
            ownerText.textContent = productData.ownerName || "";
            contactText.textContent = productData.contact || "";
            locationText.textContent = productData.location || "";
