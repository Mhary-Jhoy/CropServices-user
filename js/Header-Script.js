      ...
      if (snapshot.exists()) {
        ...
        document.addEventListener("headerLoaded", () => {
          ...
          function checkScreenWidth() {
            ...
            const headerContact = document.querySelectorAll(
              "header .header-contact"
            );

            if (screenWidth <= 600) {
              headerContact.forEach((div) => (div.style.display = "none"));
              btnMenu.style.display = "block";
            } else if (screenWidth >= 601 && screenWidth <= 768) {
              headerContact.forEach((div) => (div.style.display = "flex"));
            } else {
            }
          }
          ...
          async function displayPosterApplicant() {
            // Show the poster container and clear existing content
            ...
            if (snapshot.exists()) {
              ...
              for (const AID in applicantData) {
                ...
                // Event listener for showing applicant details
                btnApplicant.addEventListener("click", () => {
                  showApplicantDetails(data, AID);
                });
              }
            }
          }

          function showApplicantDetails(data, AID) {
            ...
            const btnDecline = inboxContainer.querySelector(
              ".btn-applicant-decline"
            );
            const btnAccept = inboxContainer.querySelector(
              ".btn-applicant-accept"
            );
            ...
            // Add new event listeners for the current applicant
            inboxContainer
          ...
          async function handleDecline(AID) {
            try {
              const applicationRef = ref(
                database,
                `farmers/${user.uid}/applications/${AID}`
              );
              ...
              if (applicationSnapshot.exists()) {
                ...
                if (status !== "Declined") {
                  await update(applicationRef, { status: "Declined" });

                  const postRef = ref(database, `farmers/${user.uid}`);
                  const postSnapshot = await get(postRef);
                  if (postSnapshot.exists()) {
                    const postData = postSnapshot.val();
                    const acceptedCount = postData.accepted || 0;
                    await update(postRef, { accepted: acceptedCount - 1 });
                  }
                }
              ...
              alert("Error declining applicant: " + error.message);
            }
          }

          async function handleAccept(AID) {
            try {
              const applicationRef = ref(
                database,
                `farmers/${user.uid}/applications/${AID}`
              ...
              if (applicationSnapshot.exists()) {
                ...
                if (status !== "Accepted") {
                  await update(applicationRef, { status: "Accepted" });
